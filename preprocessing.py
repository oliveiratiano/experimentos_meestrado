...
pontuacao = re.sub('-','',string.punctuation)
...
def preprocessar_texto(line, saida, tokens):
    linha_limpa = clean_text(line)
    for sent in sent_tokenizer.tokenize(linha_limpa):
        if sent.count(' ') >= 3 and sent[-1] in ['.', '!', '?', ';']:
            if sent[0:2] == '- ':
                sent = sent[2:]
            elif sent[0] == ' ' or sent[0] == '-':
                sent = sent[1:]            
            sent = sent.translate(str.maketrans(pontuacao, ' '*len(pontuacao)))
            sent = re.sub('\s+', ' ', sent)
            saida.write('%s\n' % sent)
            tokens += sent.count(' ') + 1
...
def main():
    ...
    f_in = 'dados/corpus_txt/corpus_raw_txt.txt'    
    f_out = 'dados/corpus_txt/corpus_pre_no_punkt.txt'
    print("Contando linhas do arquivo de entrada")
    ...
    entrada = open(f_in, 'r', encoding='utf8')
    saida = open(f_out, 'w', encoding='utf8')
    

    print("Iniciando processamento do Corpus")

    # Clean lines.
    for i, line in tqdm(enumerate(entrada)):
        preprocessar_texto(line, saida, vocab, tokens)
